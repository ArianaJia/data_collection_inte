...
这些资料已经涵盖该模块的硬件和软件调试说明，代码部分不在这里详细展开。硬件上 4G 模块通过母板上的 7Pin 接口接入，主要信号包括 `VIN_5V`、`VIN_GND`、`USART1_TX`、`USART1_RX`、`PA11`、`PA12` 和 `PF7`。其中 USART1 用于 AT 指令和数据交互，其余控制脚可按实际固件需求用于上电控制、状态检测或流控。
...
时钟部分包含 8MHz 主晶振和 32.768kHz 低速晶振。BOOT0 通过 10k 下拉，默认从主闪存启动，不适合直接依赖串口下载；BOOT1 对应 PB2，高低电平都可以，建议悬空。复位电路由 10k 上拉、100nF 电容和按键组成，SWD 接口引出 3.3V、GND、SWDIO、SWCLK，方便下载和调试。板上还放置了红、绿两个测试 LED，分别串 1k 电阻到 MCU 控制脚。
...
外部供电入口使用 24V 和地输入，电源板输出的 5V、3.3V 和地再回到母板分配给各模块。当前原理图里还预留了 `LV_IN+`、`LV_IN-`、`LV_`、`VOUT_5V`、`VOUT_GND` 等低压侧网络，方便后续把这块板扩展成更通用的电源转换和电平支撑平台。

### CAN信号扩展——MCP2518FD

CAN 控制器扩展使用 MCP2518FDTE/SL。它本质上是 SPI 到 CAN FD 控制器的转换芯片，适合在 MCU 原生 CAN 数量不够时扩展更多可控 CAN 通道。当前设计放置两片 MCP2518FD，分别作为 CANA 和 CANB 的控制器：
...
- CANA 作为控制器相关 CAN，CANB 作为 CDC 报文相关 CAN。具体命名只服务于线束识别，调试时以实际接线为准。
...
终端电阻按实际总线位置决定是否焊接。原理图中针对 CANB 预留了 120Ω 终端，其余通道也需要在整车线束拓扑确认后再决定是否补焊，避免在同一条总线上形成过多终端。
...
I2C 转接板本身不做信号处理，只负责接口转换。板上同时放置 XD-GX12-4P-M 航插和 2.54mm 1 × 4 母座，四根线序统一为：
...
24V 到 5V 使用 LM2596S-ADJ。原理图参考成熟模块电路实现，输入端放置 470uF 电解电容和 100nF 高频去耦，开关输出后接 47uH 电感，续流二极管使用 SS34-MS，输出端放置 220uF 电解电容和 100nF 去耦。
...
目前完整输出一帧温度矩阵大约 1 秒左右，与连续读取两个 subpage、计算 768 点温度、串口输出 24 行文本以及主循环延时有关。降低 MLX90640 刷新率通常可以让单帧噪声更小、画面更稳定，但不会直接消除发射率、环境反射、传感器热稳定和安装视场带来的绝对误差。
...
- `CAN1`：整车 `CANB`，对应整车状态、轮边电机状态、IMU、IVT/FS 能量计等报文。
- `CAN2`：电池箱 CAN，对应电池包扩展帧、单体电压、温度、状态量等报文。
- `SPI1 + MCP2518FD`：控制器 `CANA`，当前由 `Task06` 负责轮询与解码，服务电机控制器相关报文。
- `SPI3 + MCP2518FD`：当前规划为 CDC（半主动电磁阻尼悬架）监视总线，建议保持只监听、不主动发控制报文。
...
- 原生 CAN 走中断。`HAL_CAN_RxFifo0MsgPendingCallback()` 与 `HAL_CAN2_RxFifo0MsgPendingCallback()` 在中断里只做“收帧 + 入队”，避免在中断上下文里做复杂解码。
- MCP2518 走轮询。`Task06` 周期性调用 `FDCAN_Poll()`，读取 `SPI1/CANA` 的 FIFO 帧后直接完成控制器报文解码；`Task07` 作为 `SPI3/CDC` 的独立监听任务，负责只读监视，并将最近一帧写入 `g_CDCInfo`，不并入电池箱链路。
...
因此，后续新增标准 CAN 报文时，优先考虑的不是“它来自 bxCAN 还是 MCP2518FD”，而是“它属于哪个业务网络，由哪个任务或 `can_channel` 进入哪个解码函数”。当前 `CANA` 已经明确归属于 `Task06`，不再并入 `Task02`。
...
- `EN` 引脚：用于 485 芯片方向/使能控制，该引脚在 `Task05` 启动时被初始化到默认状态。
- `Task05_ParseRs485Message()`：预留的协议解析入口。
- `Peripheral_Rx_Frame_t`：通用外设接收帧封装，用于承接字节流或未来的完整帧。

当前 `StartTask05()` 的实现是最小可运行版：循环从 `USART2` 逐字节读取数据，然后把每个字节封装进 `Peripheral_Rx_Frame_t` 送到 `Task05_ParseRs485Message()`。这个设计的优点是入口已经稳定，后续只需要替换解析逻辑，不需要再改任务调度结构。

但也正因为当前实现是逐字节喂入，所以后续如果正式接入悬架拉力/位移类传感器，建议在 `Task05_ParseRs485Message()` 内部增加完整的状态机，而不要在任务外层直接拼帧。推荐结构如下：
...
- `Task05_ParseRs485Message()` 只负责解析返回帧，不负责决定轮询顺序。
...
1. `Task05` 负责收字节流；
2. `Task05_ParseRs485Message()` 负责协议解码；
3. 解码结果写入 `telemetry_data`；
4. `publish.c` 决定是否对外发布。

不要让 `Task05` 直接去拼 protobuf，也不要让 `publish.c` 去碰协议字节。协议层和对外发布层一旦混在一起，后续更换传感器型号时维护成本会非常高。

## 4G传输——基于protobuf的MQTT订阅

当前 MCU 侧的 4G 发送链路，应当理解为“protobuf 序列化 + UART1 发送”，而不是“MCU 内部完整 MQTT 协议栈”。这点对后续维护非常重要。
...
3. 用项目自定义帧头包一层，再经 `USART1` 送给 4G 模块。
...
其中 `Publish_Process()` 只负责“何时把某个 topic 入队”，真正的编码和串口发送由 `Publish_TxTaskStep()` 负责执行。这样做的好处是把“调度”和“发送”拆开，避免主调度逻辑因为一次编码或串口忙而失控。

当前完整链路如下：

```text
decode result in telemetry_data
 -> Publish_Process() decides topics
 -> myQueue04 stores PublishQueueItem_t
 -> Publish_TxTaskStep() dequeues one topic
 -> Publish_Map...() fills protobuf structs
 -> pb_encode()
 -> Publish_SendFrame()
 -> USART1
 -> 4G module
...
这里还需要强调一个你已经明确修改过的设计点：当前 `publish` 路径不再依赖额外的 485 使能脚控制，发送就是直接走 `USART1`。因此以后更换 4G 模块时，只要模块仍然接受 UART 数据，就优先保持 `publish.c` 不动，把 AT 配置、联网、MQTT 上线等控制逻辑留在模块初始化或外部网关侧。

当前 MCU 自定义发送帧格式为：

```text
MAGIC0 MAGIC1 topic_len payload_len_l payload_len_h topic_name payload
```

也就是说，4G 模块或上位机侧如果要继续兼容当前固件，不能只会解 protobuf，还必须先识别这层自定义外包头。
...
- `defaultTask`：系统主调度任务，执行 `Publish_Process()`、`Publish_TxTaskStep()`、调试串口输出和状态灯翻转。
- `Task02`：处理 `myQueue02`，仅负责整车 `CANB` 解码入口。
- `Task03`：处理 `myQueue03`，负责电池箱 CAN 解码入口。
- `Task04`：热成像采集与摘要生成任务。
- `Task05`：RS485 入口任务。
- `Task06`：`SPI1 + MCP2518FD` 轮询与 `CANA` 解码任务。
- `Task07`：`SPI3 + MCP2518FD` 轮询与 CDC 监视任务。
- `Task08`：`USART1` 发送任务。
- `Task09`：`CAN1` 发送任务。

对应的队列设计如下：

- `myQueue01`：USART1 发送队列
- `myQueue02`：CAN1 收到的整车 `CANB` 业务帧队列
- `myQueue03`：当前主要承接 CAN2 的电池箱业务帧队列
- `myQueue04`：publish topic 队列
- `myQueue05`：CAN1 发送队列
...
- `Task02/Task03/Task06` 当前已经稳定写入 `g_CANB_LoopData` / `g_BatteryInfo`
- `Task07` 当前作为 CDC 监视入口存在，已经写入独立的 `g_CDCInfo` 通用缓存；后续若明确 CDC 报文语义，再在该缓存基础上补充协议专属字段，而不是复用电池箱或整车缓存
- `Task04` 最终写 `g_MLX90640_Frame`
...
- 业务发布：走 `myQueue04 / myQueue01 -> huart1`

这样可以避免调试信息污染正式对外数据口，也避免发布链路阻塞时把调试一起拖死。

热成像任务 `Task04` 是现阶段多任务设计最完整的一个示例。它做了这些事：
...
- 如果它是“原生 CAN 中断收包”，放到 `Task02/Task03` 的既有链路里；
- 如果它是“SPI 外接控制器轮询收包并且业务上独立于原生 CAN”，优先放到 `Task06/Task07`；
- 如果它是“I2C 主动轮询采集”，优先仿照 `Task04`；
- 如果它是“串口字节流协议”，优先仿照 `Task05`；
...
3. 在 `Task05_ParseRs485Message()` 内增加独立解析分支；
